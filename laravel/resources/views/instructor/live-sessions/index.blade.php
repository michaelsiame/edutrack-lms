                ...
                <div class="flex items-start justify-between">
                    ...
                    <div class="flex items-center gap-2">
                        @if($session->isLive() || $session->isUpcoming())
                            <a href="{{ route('student.live-sessions.join', $session) }}" target="_blank" class="px-3 py-1.5 bg-green-600 text-white text-xs rounded-lg hover:bg-green-700 font-medium">
                        ...
                        <form action="{{ route('instructor.live-sessions.destroy', [$course, $session]) }}" method="POST" onsubmit="return confirm('Delete this session?')">
                            @csrf @method('DELETE')
                            <button type="submit" class="px-3 py-1.5 bg-red-100 text-red-700 text-xs rounded-lg hover:bg-red-200 font-medium">
