            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-5">
                <div class="flex items-start gap-4">
                    <div class="w-12 h-12 rounded-full bg-amber-100 dark:bg-amber-900 flex items-center justify-center flex-shrink-0">
                        <i class="{{ $badge->badge_icon_url }} text-amber-600 dark:text-amber-400 text-lg"></i>
                    </div>
                    <div class="flex-1">
                        ...
                        <div class="flex items-center gap-2 mt-3">
                            <form action="{{ route('admin.badges.toggle', $badge) }}" method="POST">
                                @csrf
                                <button type="submit" class="text-xs px-2 py-1 rounded {{ $badge->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' }}">
                                    {{ $badge->is_active ? 'Active' : 'Inactive' }}
                                </button>
                            </form>
                            <form action="{{ route('admin.badges.destroy', $badge) }}" method="POST" onsubmit="return confirm('Delete this badge?')">
                                @csrf @method('DELETE')
                                <button type="submit" class="text-xs text-red-600 hover:text-red-700"><i class="fas fa-trash"></i></button>
                            </form>
                        </div>
                    </div>
                </div>
