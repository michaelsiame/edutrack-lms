        ...
        <div class="flex items-center justify-between mb-4">
            ...
            <div class="text-right">
                @if($assignment->due_date)
                    <p class="text-sm {{ $assignment->due_date->isPast() && !$submission ? 'text-red-600' : 'text-gray-500' }}">
            ...
            <div class="space-y-4">
                <div class="flex items-center gap-4">
                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium
                        {{ $submission->status === 'Graded' ? 'bg-green-100 text-green-800' : ($submission->status === 'Late' ? 'bg-amber-100 text-amber-800' : 'bg-blue-100 text-blue-800') }}">
                        {{ $submission->status }}
                    </span>
                    ...
                    <div class="border-t border-gray-100 dark:border-gray-700 pt-4">
                        ...
                        @if($submission->feedback)
                            <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-lg p-4">
                                <h4 class="font-medium text-blue-900 dark:text-blue-300 mb-1">Instructor Feedback</h4>
                                <p class="text-sm text-blue-800 dark:text-blue-400">{{ $submission->feedback }}</p>
                            </div>
                        @endif
                    </div>
                @endif
