        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <select name="question_type" id="question_type" x-model="questionType"
                    ...
                    <option value="Fill in Blank" {{ $question->question_type === 'Fill in Blank' ? 'selected' : '' }}>Fill in Blank</option>
                </select>
            </div>
            <div>
...
{{ old('correct_answer', $question->correct_answer) }}</textarea>
...
function questionForm() {
    return {
        ...
        correctIndex: {{ old('correct_option_index', $question->options->search(function($opt) { return $opt->is_correct; }) ?? 0) }},
        correctAnswer: '{{ old('correct_answer', $question->correct_answer ?? '') }}',
        options: {!! json_encode(old('options', $question->options->map(fn($opt) => ['text' => $opt->option_text])->values()->toArray())) !!},
        get hasOptions() {
