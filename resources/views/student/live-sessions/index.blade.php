...
<div class="max-w-4xl mx-auto">
 ...
 @if($liveSession)
 <div class="mb-6 bg-danger-50 border border-danger-200 rounded-xl p-6">
 <div class="flex items-center justify-between">
 <div class="flex items-center gap-3">
 <div class="w-3 h-3 bg-danger-600 rounded-full animate-pulse"></div>
 <div>
 <h3 class="font-bold text-danger-800">LIVE NOW</h3>
 <p class="text-sm text-danger-600">{{ $liveSession->description }}</p>
 </div>
 </div>
 <a href="{{ route('student.live-sessions.join', $liveSession) }}" target="_blank"
 class="px-6 py-2 bg-danger-600 text-white rounded-lg hover:bg-danger-700 font-medium">
 ...
 @if($session->isLive())
 <a href="{{ route('student.live-sessions.join', $session) }}" target="_blank" class="px-4 py-2 bg-danger-600 text-white text-sm rounded-lg hover:bg-danger-700 font-medium">
 ...
 <a href="{{ $session->recording_url }}" target="_blank" class="px-4 py-2 bg-primary-100 text-primary-700 text-sm rounded-lg hover:bg-primary-200 font-medium">
